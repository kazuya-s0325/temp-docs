...
- [4. FEシステム単体での動作保証](#4-feシステム単体での動作保証)
  - [FEモジュールの自律性に関する指針](#feモジュールの自律性に関する指針)
...
### 1ファイルあたりの行数
- **[SHOULD]** 1ファイルあたり300～500行くらいに収める（ただし、空行とコメントは行数にカウントしない）
...
- **[SHOULD]** **FEモジュール単体での動作を保証**すること
  - Backend APIが未実装・停止状態であっても、最低限のUIレンダリング・操作確認ができるようにするため
  - BEが存在しないと動かないFEは、テスト容易性・開発効率が著しく低下する

## FEモジュールの自律性に関する指針
- **[SHOULD]** 以下の指針でFEモジュールの自律性を担保する

  | 項目               | 内容                                                                                                                                                                                       |
  | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
  | API層の抽象化      | APIラッパー群では、内部でaxiosを直接叩くのではなく、`apiClient` などの共通ラッパーを経由<br>これらは `DI` (dependency injection) で切り替え可能な構造にする                                |
  | Mock対応           | FEモジュールは、可能な限りMockデータを使用して単独動作可能であること<br>API未接続状態でも画面表示・UI挙動を確認できるようにする                                                            |
  | デフォルト値の定義 | APIレスポンスを前提とせず、初期描画に必要な最低限のデフォルト値を定義すること<br>また、例えば `v-if` で要素ごと消すのではなく、`loading` や `empty` 等といった状態を適切にハンドリングする |
  | データ依存の明示化 | `composable` や `store` を通してAPI依存部分は明確に切り出すこと<br>UIコンポーネントは表示のみに専念し、通信やデータ変換処理などは直接行わない                                              |
  | エラーハンドリング | BEの応答に依存するケースでは、非同期エラーやタイムアウト時の挙動を定義すること<br>「何も表示されない」はNG、ユーザーに意図を伝える                                                         |

- **[SHOULD]** 初期化処理部分に関する指針
  | 区分             | 内容                                                                                              |
  | ---------------- | ------------------------------------------------------------------------------------------------- |
  | 初期表示処理     | 画面初期化は原則、FE側で完結させること<br>API依存による「初期データ取得」は避けること             |
  | マスタデータ     | マスタ取得値は必要に応じて、`store`へキャッシュを行う                                             |
  | サーバー呼び出し | 初期表示でAPIを呼ぶ場合は明確な理由を説明すること                                                 |
  | Mock対応         | 初期化用のAPIレスポンスに依存する場合は、ローカルビルドで代替可能な`mock`データを必ず用意すること |
  | 単独動作性       | FE単体でビルド・表示可能であるようにする（BE未接続でエラーとなり操作できない、という設計はNG）    |
